...
with PdfPages(REPORT) as pdf:
    ...
    content = [
        ...
        ("DATA & UNIVERSE", [
            ...
            "Universe: ~700 S&P 500 stocks, 108 months total (Jan 2016 – Dec 2024)",
        ...
        ("METHODOLOGY", [
            "Signal: XGBoost (n_estimators=200, learning_rate=0.05, max_depth=4)",
            "Features (11 total):",
            "  • FF4 betas (4): beta_mktrf, beta_smb, beta_hml, beta_mom",
            "  • Option-implied (3): SKEW (skewness), AIV (atm iv), GLB (global beta)",
            "  • Traditional (3): Mom12m (12-month momentum), IdioVol3F (idio vol), BM (book-to-market)",
            "  • Residual momentum (1): resid_signal",
            "Preprocessing: Winsorize(p1/p99) → Box-Cox (λ fitted on IS) → Z-score (cross-section per month)",
            "Optimizer: cvxpy maximize(μᵀw) s.t. Σw=1, 0≤wᵢ≤2·w_bench,i, |βQS−βBNCH|≤0.10, TE≤1.5%",
            "Covariance: Ledoit-Wolf shrinkage, 36-month rolling window",
        ]),
        ("KEY FINDINGS", [
            "✦ Full-period: Ann. Return 17.7% vs 14.1% benchmark (+3.6%); Sharpe 1.09 vs 0.93",
            "✦ Full-period Information Ratio: 1.26 (NW-adjusted); Tracking Error: 2.87%",
            "✦ IS outperformance strong: Ann. Return 22.8% vs 16.3% (+6.5%); IR: 2.35",
            "✦ OOS performance: ~0% excess return (11.39% vs 11.39% benchmark)",
            "  → IS IC = +0.19 collapses to OOS IC = −0.025 (signal decay)",
            "✦ Box-Cox+Z preprocessing improved OOS excess return from −1.7% to ≈0%",
            "  vs Rank-Normalization (OOS IR: −0.0001 vs −0.74 with Rank-Norm)",
            "✦ Option-implied features most regime-stable (OOS IC: −0.009 vs −0.025 all features)",
            "✦ All 3 constraints satisfied 100% of the time across 108 months",
            "✦ Max monthly underperformance: −1.38% (within −2% drawdown constraint)",
            "✦ Limitation: OpenAssetPricing data gap in 2023–2024; no transaction costs modeled",
        ]),
    ]
    ...
    method_sections = [
        ...
        ("2. Signal Generation (XGBoost)", [
            "Model: XGBoost gradient boosting tree (n_estimators=200, learning_rate=0.05,",
            "  max_depth=4, subsample=0.8, colsample_bytree=0.8, random_state=42)",
            "Target: Next-month cross-sectional rank of stock returns",
            "Walk-forward: IS model trained on 2016–2020; OOS re-estimated monthly using",
            "  expanding window (each month adds new observations). Strict no-lookahead enforced.",
            "Assertion: max(train_date) < min(predict_date) verified at each step.",
        ]),
        ...
        ("5. Portfolio Optimization", [
            "Objective: maximize μᵀw  (expected return given XGBoost signal scores)",
            "Constraints:",
            "  • Σwᵢ = 1  (fully invested)",
            "  • 0 ≤ wᵢ ≤ 2 × w_bench,i  (no short, max 2× benchmark weight)",
            "  • |β_QS,k − β_BNCH,k| ≤ 0.10  for k ∈ {MktRF, SMB, HML, MOM}",
            "  • Tracking Error (TE) ≤ 1.5%  (risk budget via covariance constraint)",
            "Covariance: Ledoit-Wolf shrinkage estimator, 36-month rolling window",
            "Solver: ECOS (default in cvxpy) with fallback to SCS",
        ]),
    ...
    disc_sections = [
        ("IS Overfitting Analysis", [
            "The strategy exhibits a classic IS/OOS performance gap. In-sample (2016–2020), the",
            "XGBoost model achieves an IC of +0.19 and annualized excess return of +6.5%, indicating",
            "strong signal capture during the IS period. However, OOS IC collapses to −0.025 and",
            "excess return converges to ~0%, suggesting that the learned patterns do not generalize.",
            "",
            "Root causes identified:",
            "  (a) Regime shift: 2020–2024 included COVID shock, post-COVID inflation, and",
            "      Federal Reserve rate hike cycle — regimes not present in IS period.",
            "  (b) Signal crowding: Residual momentum and traditional factors are widely deployed;",
            "      alpha decay is expected as strategies become more competitive.",
            "  (c) Data availability: OpenAssetPricing features have a coverage gap in 2023–2024,",
            "      forcing the model to use stale or imputed feature values.",
        ]),
        ("Preprocessing Impact", [
            "Box-Cox+Z-Score preprocessing improved OOS performance significantly relative to",
            "Rank-Normalization. By fitting the power transformation parameters λ on IS data and",
            "applying them frozen in OOS, the pipeline reduces feature distributional shift.",
            "The OOS excess return improved from −1.7% (Rank-Norm) to ≈0.0% (Box-Cox+Z),",
            "and the Information Ratio improved from −0.745 to −0.000.",
            "",
            "This demonstrates that preprocessing is not a neutral step — distributional",
            "assumptions embedded in the normalization method can have material performance impact.",
        ]),
        ("Option-Implied Feature Stability", [
            "Among feature subsets, option-implied features (SKEW, AIV, GLB) show the best OOS",
            "stability with OOS IC = −0.009 vs −0.025 for the full feature set. This aligns with",
            "academic literature suggesting that option-implied signals contain forward-looking",
            "information from informed traders that is less susceptible to regime changes.",
            "",
            "Future work: An option-implied-only model with regime-adaptive signal weighting",
            "may achieve positive OOS IC. This is left for subsequent research.",
        ]),
        ("Constraint Effectiveness", [
            "All three hard constraints — weight bounds, factor exposure, and monthly active return",
            "— were satisfied 100% of the time across 108 months. The maximum monthly",
            "underperformance was −1.38%, well within the −2% limit.",
            "",
            "The factor exposure constraints (|Δβ| ≤ 0.10) effectively limit unintended factor",
            "tilts while still allowing the optimizer to exploit the XGBoost signal. The tracking",
            "error constraint (TE ≤ 1.5%) ensures the portfolio does not deviate excessively",
            "from the benchmark even when signal quality is low.",
        ]),
        ("Limitations & Future Work", [
            "• No transaction costs modeled — monthly rebalancing of 700 stocks would incur",
            "  significant turnover costs in practice. Net-of-cost IR would be lower.",
            "• Data gap: OpenAssetPricing feature coverage drops in 2023–2024, creating",
            "  survivorship and look-ahead risks in late OOS period.",
            "• Single rebalancing frequency: Monthly may not be optimal. Bi-weekly or adaptive",
            "  rebalancing based on signal confidence could improve efficiency.",
            "• Future: Regime-adaptive λ refitting, online learning for XGBoost, and ensemble",
            "  signal combination (e.g., option-implied + fundamental) are promising directions.",
        ]),
        ("Conclusion", [
            "This study presents a rigorous academic evaluation of a constrained long-only portfolio",
            "combining XGBoost ML signals with traditional and option-implied features. The strategy",
            "achieves strong IS performance (IR: 2.35, +6.5% excess return) and a respectable full-",
            "period IR of 1.26. OOS excess return is approximately zero, reflecting a challenging",
            "post-2020 regime. All three portfolio constraints are satisfied 100% of the time.",
            "",
            "The key contribution is the systematic evaluation of preprocessing pipelines, showing",
            "that Box-Cox+Z-Score normalization provides meaningful OOS improvement over rank-based",
            "methods. This project provides a clean, reproducible framework for future research.",
        ]),
    ]
